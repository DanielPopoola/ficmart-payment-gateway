...
func NewCaptureService(repo repository.PaymentRepository, bankClient bank.BankPort) *CaptureService {
...
func (c *CaptureService) Capture(ctx context.Context, paymentID uuid.UUID, amount int64, idempotencyKey string) (*domain.Payment, error) {
	...
	err := c.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
	...
	updateErr := c.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
		p, err := txRepo.FindByIDForUpdate(ctx, paymentID)
		if err != nil {
			return err
		}
...
func (c *CaptureService) Reconcile(ctx context.Context, p *domain.Payment) error {
	...
	return c.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
