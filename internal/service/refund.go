...
func NewRefundService(repo repository.PaymentRepository, bankClient bank.BankPort) *RefundService {
...
func (r *RefundService) Refund(ctx context.Context, paymentID uuid.UUID, amount int64, idempotencyKey string) (*domain.Payment, error) {
	...
	err := r.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
	...
	updateErr := r.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
		p, err := txRepo.FindByIDForUpdate(ctx, paymentID)
		if err != nil {
			return err
		}
...
func (r *RefundService) Reconcile(ctx context.Context, p *domain.Payment) error {
	...
	return r.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
