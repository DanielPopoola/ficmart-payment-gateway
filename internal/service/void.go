...
func NewVoidService(repo repository.PaymentRepository, bankClient bank.BankPort) *VoidService {
	return &VoidService{
		repo:       repo,
...
func (v *VoidService) Void(ctx context.Context, paymentID uuid.UUID, idempotencyKey string) (*domain.Payment, error) {
	...
	err := v.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
	...
	updateErr := v.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
		p, err := txRepo.FindByIDForUpdate(ctx, paymentID)
		if err != nil {
			return err
		}
...
func (v *VoidService) Reconcile(ctx context.Context, p *domain.Payment) error {
	...
	return v.repo.WithTx(ctx, func(txRepo repository.PaymentRepository) error {
